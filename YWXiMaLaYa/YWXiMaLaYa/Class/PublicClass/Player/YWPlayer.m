...
@property (nonatomic, strong) YWResourceLoader * resourceLoader;

@property (nonatomic, strong) id timeObserve;

@end

@implementation YWPlayer
...
- (void)reloadCurrentItem {
    ...
    if ([self.url.absoluteString hasPrefix:@"http"]) {
        //有缓存播放缓存文件
        NSString * cacheFilePath = [YWFileHandle cacheFileExistsWithURL:self.url];
        if (cacheFilePath) {
            ...
            //没有缓存播放网络文件
            self.resourceLoader = [[YWResourceLoader alloc]init];
...
- (void)play {
    if (self.state == YWPlayerStatePaused || self.state == YWPlayerStateWaiting) {
        [self.player play];
    }
}

- (void)pause {
    if (self.state == YWPlayerStatePlaying) {
        [self.player pause];
    }
}

- (void)stop {
    if (self.state == YWPlayerStateStopped) {
        return;
    }
    [self.player pause];
    ...
    self.progress = 0.0;
    self.duration = 0.0;
    self.state = YWPlayerStateStopped;
}

- (void)seekToTime:(CGFloat)seconds {
    if (self.state == YWPlayerStatePlaying || self.state == YWPlayerStatePaused) {
        [self.player pause];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSString *,id> *)change context:(void *)context {
    ...
    if ([keyPath isEqualToString:@"rate"]) {
        if (self.player.rate == 0.0) {
            _state = YWPlayerStatePaused;
        }else {
            _state = YWPlayerStatePlaying;
        }
    }
}
...
- (void)setState:(YWPlayerState)state {
    [self willChangeValueForKey:@"progress"];
    _state = state;
    [self didChangeValueForKey:@"progress"];
}
